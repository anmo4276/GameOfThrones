#include <iostream>
#include <string>
#include "Hero.h"
#include <fstream>
using namespace std;
int split1(string phrase, char splitChar, string store[],int length)
{
    int count=0; //Initializes a variable of type integer named count and sets it equal to 0
    int i =0; //Initializes a variable of type integer named i and sets it equal to 1
    int index=0;
    store[length];
    if(phrase.length()!=0) //If the string length is not 0
    {
        count=1; //The count is one
    }
    else if(phrase.length() == 1 && phrase[0]!= splitChar) //If string length is one and its not a delimeter
    {
        count = 0; //The count is 0
    }
    if(splitChar == phrase[0]) //If the first character is a delimeter
    {
        i =1; //Start the for loop at 1
    }
    for (i; i < phrase.length(); i++) //For loop for the length of the string
        {
         if (i == phrase.length()- 1) //If the for loop is on the last character
         {
             if(phrase[i]!=splitChar) // If he last character isn't a delimeter
             {
                  store[index]=store[index]+phrase[i]; //Store the character at i into the array at index
             }
             break; //Exi the for loop
         }
         else if (phrase[i]==splitChar && phrase[i+1]!=splitChar) //Checks if there aren't two delimeter in a row
         {
                 index++; //Add one to index
                 count++; //Adds one to string count
         }
         else if(phrase[i]==splitChar)//If the character at i is a delimeter
         {
             if (phrase[i]==splitChar && phrase[i- 1]!=splitChar)//If the string at i is a delimeter and the one before it isnt:
             {
                 //Do nothing
             }
             else //Else
             {
             if(i!=0)//If it isn't the first character
             {
             index++; //SAdd one to index
             count++; //Add one to count
             }
             }
         }
         else //Else
         {
             store[index] = store[index] + phrase[i]; //Store the character at i into the array at index
         }
         
        }   
        return count; //Return the string count
}
        Hero:: Hero(){
            name = "";
            money =0;
            influence=0;
            armySize =0;
            positionRow=0;
            index =0;
            positionColumn=0;
            ship = "";
        }
        void Hero::setHeroName(string namePublic){
            name = namePublic;
        }
        string Hero::getHeroName(){
            return name;
        }
        void Hero::setHeroMoney(int moneyPublic){
            money = moneyPublic;
        }
        int Hero::getHeroMoney(){
            return money;
        }
        void Hero::setWarriors(string warriorsPublic[]){
            int warriorIndex=0;
            string store2[7];
            ifstream myFile;
            for(int b =0; b<4;b++){
                warriors[b].setWarriorName(warriorsPublic[b]);
            
            if(warriorsPublic[b]!= "NULL"){
                myFile.open("warriorsGOT.txt");
                 if (myFile.is_open()) //If file opens:
                {
                    string line2 = ""; //Line equals ""
        
        
                    while (getline(myFile, line2))  //For every line in the file:
                    {
                    
                        if(!line2.empty()) //If line isn't empty:
                            {
                                split1(line2,',',store2,7);
                                if(store2[0]== warriors[warriorIndex].getWarriorName()){
                                    warriors[b].setWarriorStrength(stoi(store2[1]));
                                    warriors[b].setWarriorLoyalty(stoi(store2[2]));
                                    warriors[b].setWarriorMorale(stoi(store2[3]));
                                    warriors[b].setWarriorFree(store2[4]);
                                    warriors[b].setWarriorShip(store2[5]);
                                    warriors[b].setWarriorDragonlass(store2[6]);
                                }
                                
                                for(int v=0; v<7;v++)
                                {
                                    store2[v] = "";
                                }
                            }
                            
                    }
                }
               
            }
            }
             myFile.close();
        }
        Warrior Hero::getWarriors(int index){
            return warriors[index];
        }
        void Hero::setWarriorStats(int index, Warrior newWarrior){
            warriors[index] = newWarrior;
        }
        void Hero::setHeroInfluence(int influencePublic){
            influence = influencePublic;
        }
        int Hero::getHeroInfluence(){
            return influence;
        }
        void Hero::setArmySize(int armySizePublic){
            armySize = armySizePublic;
        }
        int Hero::getArmySize(){
            return armySize;
        }
        void Hero::setHeroLocation(int positionRowPublic, int positionColumnPublic){
            positionRow = positionRowPublic;
            positionColumn = positionColumnPublic;
        }
        int Hero::getPositionRow(){
            return positionRow;
        }
        int Hero::getPositionColumn(){
            return positionColumn;
        }
        void Hero::setShip(string shipPublic){
            ship = shipPublic;
        }
        string Hero::getShip(){
            return ship;
        }
        void Hero::setHeroPoints(int pointsPublic){
            points = pointsPublic;
        }
        int Hero::getHeroPoints(){
            return points;
        }
        